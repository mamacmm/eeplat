...
public class CoreSaveEditAction extends DOAbstractAction {
	...
	public String excute() {
		...
		if (insertService == null) {// updateService
		...
		try {
			...
			System.out.println("All + Rules:::" + insertService.retrieveServiceRules());
			...
			if (updateService != null) {
				...
				if (ids != null && ids.length > 0) {
					...
					PreparedStatement pstmt = con
							.prepareStatement(updateService.getTempSql());
					for (int ii = 0; ii < ids.length; ii++) {
						...
						if (idstr != null) {
							...
							BOInstance oldInstance = refreshBO
									.refreshContext(id);
							...
							for (Iterator it = updateService
								...
								value = dops.getAfterPattermValue(value);
								updateService.putStatementAValue(pstmt, i,
										dops, value);
								i++;
							}
							batchSize++;
