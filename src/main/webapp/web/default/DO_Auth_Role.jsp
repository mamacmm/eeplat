...
<!-------------------------------- 工程权限定义 ----------------------------------------------------->                      
                      <TR bgcolor="#e6EEEE" > 
                        <TD colspan="2" ><b>:: <%= I18n.instance().get("工程权限定义")%> ::</b></TD>
                      </TR>
  <%
 
 			 List filterApps = DOApplication.getApplicationsNoAuth();
			 for(Iterator it = filterApps.iterator(); it.hasNext();){
				 DOApplication anApp = (DOApplication)it.next();
				 String checked = "";
				 if(anApp.isAccessByRole(roleUid)){
					 checked = " checked='checked' ";
				 }
 %>
                      <TR> 
                        <TD bgColor=#ffffff width="141" height="32"><%=anApp.getL10n() %>
                        </TD>
                        <TD  bgColor=#ffffff width="620" height="32"> 
                             <input type="checkbox" name="appUid"  class="appUid"  id="a<%=anApp.getObjUid()%>" <%=checked%>  value="<%=anApp.getObjUid()%>">
                        </TD>
                      </TR>
           <%} %>
<!-------------------------------- 工程权限定义 ----------------------------------------------------->                      
...
<!-------------------------------- 菜单权限定义 ----------------------------------------------------->                      
  ...
  <%
	...
	if (exedoContext != null && exedoContext.getUser() != null) {
	   ...
	   String default_app_uid = exedoContext.getUser().getValue("default_app_uid");
	   
	   DOApplication  doa = DOApplication.getApplicationByID(default_app_uid);
	   

	   if(doa!=null){
		  doaName = doa.getName(); 
	   }
	   System.out.println(doaName);
	   
	
	
	}
 
	 		List parents  = DOMenuModel.retrieveAllProjectsChildrenNoAuth(); //getMenuModelByName(doaName +  "_root");
