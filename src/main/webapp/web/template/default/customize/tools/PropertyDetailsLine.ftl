		...
		<form name="form1">
				...
				<table class='tablesorter'  border="0" cellpadding="1" cellspacing="1" >
						...
						<tr>
							<td width='25%'>${i18n('表达式：')}</td>
							<td  align='left'>
								<textarea cols="40" rows="2" name='condition'
								id='condition' title="${i18n('表达式：')}"
								 /></textarea></td>
						</tr>
						
						<tr>
							<td width='25%'>${i18n('界面显示：')}</td>
							<td  align='left'>
 ...
 <script>
				...
				function setValues(form1){
					...
					if(selectedLineTxt){
					   	...
					   	alert("${i18n('系统平台暂时出现故障，我们正在进行修复。请耐心等待并稍后重新刷新页面。如故障重复出现，请联系您的客户服务代表。')}");
					   	}
					   	window.close();
