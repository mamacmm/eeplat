	...
	_attachSVG: function(container, settings) {
		...
		try {
			...
			if ($.browser.msie) {
				...
				container.innerHTML = '<embed id="embedblanksvg" type="image/svg+xml" width="100%" ' +
...
//alert("Data::" + $('#embedblanksvg')[0].getSVGDocument());
//alert("DOC::" + wrapper._svg.ownerDocument);
			var newNode = document.getElementById('embedblanksvg').window.parseXML(data,wrapper._svg.ownerDocument);
			wrapper._svg.appendChild(newNode);
//			alert("NewNode:::" + newNode);
		...
		var loadXml4Fire = function(data){
			
			wrapper._svg.appendChild(data);
			if (settings.onLoad) {
				settings.onLoad.apply(wrapper._container || wrapper._svg, [wrapper]);
			}
			return newNode;
		...
		//			alert(newNode);
						...
						//		for (var j = 0; j < newNode.childNodes[0].attributes.length; j++) {
								//	var attr = newNode.childNodes[0].attributes.item(j);
										//	alert("NodeValue::" + attr.nodeValue);
								//}
		...
		if (url.match('<svg')) { // Inline SVG
            ...
            var xmlData =oParser.parseFromString(xml, "text/xml"); 
					  loadSVG(xmlData);						
					}
