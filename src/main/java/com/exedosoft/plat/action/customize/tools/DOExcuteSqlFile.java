...
public class DOExcuteSqlFile extends DOAbstractAction {
	...
	public String excute() {
		...
		if("mysql".equalsIgnoreCase(dss.getDialect())){
			fileName = prefix + "/web/initsql/mysql.sql";
		}else if ("sqlserver".equalsIgnoreCase(dss.getDialect())){
			fileName = prefix + "/web/initsql/sqlserver2000.sql";
		}
		else if ("oracle".equalsIgnoreCase(dss.getDialect())){
			fileName = prefix + "/web/initsql/oracle.sql";
		}else if ("gae".equalsIgnoreCase(dss.getDialect())){
			fileName = prefix + "/web/initsql/gae.sql";
		}
		else if ("db2".equalsIgnoreCase(dss.getDialect())){
			fileName = prefix + "/web/initsql/db2.sql";
		}
		else if ("h2".equalsIgnoreCase(dss.getDialect())){
			fileName = prefix + "/web/initsql/h2.sql";
		}
		else if ("hsqldb".equalsIgnoreCase(dss.getDialect())){
			fileName = prefix + "/web/initsql/hsqldb.sql";
		}
		else if ("postgresql".equalsIgnoreCase(dss.getDialect())){
			fileName = prefix + "/web/initsql/postgresql.sql";
		}
		else{
			this.setEchoValue(I18n.instance().get("现在只支持mysql、oracle、sqlserver、db2、postgresql、hsqldb、h2和gae等数据库初始化!"));
			return NO_FORWARD;
		}
