...
webFXTreeConfig.loadingIcon = "../xp/loading.gif";
...
WebFXLoadTree.documentLoaded = function (jsNode) {
	...
	try{
			...
			if(!doc || doc.parserError && doc.parseError.errorCode != 0 || !doc.documentElement) {
				...
				for (var i = 0; i < l; i++) {
					if (cs[i].tagName == "tree") {
						var newNode = WebFXLoadTree.createItemFromElement(cs[i]);
						///////////////add by weikx at 20100222			
					     try{
					        if(oldSelectName!="" && newNode.text == oldSelectName){
					           newNode.select();
					           oldSelectName="";
					        }
					     }catch(e){
