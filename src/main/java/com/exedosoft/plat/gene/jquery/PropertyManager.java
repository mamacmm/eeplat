...
public class PropertyManager {
	...
	public void removeProperty(DOBO aDOBO, DOBOProperty dop) {
		...
		try {
			if (dop != null) {
				...
				List<DOParameter> paras = DOParameter
						.getParameterByPropertyUid(dop.getObjUid());
				...
				DOService deletes = DOService
						.getService("DO_Parameter_Service_deleterubbish");
				deletes.invokeUpdate();

				// ////////////////////表格元素
				for (int i = 0; i < 10; i++) {
					DOFormModel aFm = DOFormModel.getFormModelByProperty(dop
							.getObjUid());
					if (aFm != null) {
	...
	public void addProperty(DOBO aDOBO, String colName, int type, int size,
			int scale) {
		addProperty(aDOBO, colName, colName, type, size, scale);
	}
	 ...
	 * @param type
	 */
	public void addProperty(DOBO aDOBO, String colName, String l10n, int type,
			int size, int scale) {
		...
		try {
			...
			pro.setDbType(type);
			pro.setDbSize(size);
			pro.setConstraint(String.valueOf(scale));
			pro.setDoBO(aDOBO);
			...
			DOGridModel gmBrowse = DOGridModel.getGridModelByName("GM_"
					+ aDOBO.getName() + "_browse");
			if (gmBrowse != null) {
				geneForm(pro, gmBrowse);
			}

			DOGridModel gmUpdate = DOGridModel.getGridModelByName("GM_"
					+ aDOBO.getName() + "_update");
			if (gmUpdate != null) {
				geneForm(pro, gmUpdate);
			}

			DOGridModel gmList = DOGridModel.getGridModelByName("GM_"
					+ aDOBO.getName() + "_list");
			if (gmList != null) {
				geneForm(pro, gmList);
			}

			DOGridModel gmInsert = DOGridModel.getGridModelByName("GM_"
					+ aDOBO.getName() + "_insert");
			if (gmInsert != null) {
				geneForm(pro, gmInsert);
			}

			DOGridModel gmDulplicate = DOGridModel.getGridModelByName("GM_"
					+ aDOBO.getName() + "_dulplicate");
...
//			}
	...
	public void buildRelation(String propertyName, DOBO aBO) {
		...
		t.begin();
		List<DOBOProperty> lists = DAOUtil.INSTANCE().select(
				DOBOProperty.class,
				"select * from do_bo_property  where col_name = ?",
				propertyName);
		...
		try {
			for (Iterator<DOBOProperty> it = lists.iterator(); it.hasNext();) {
				...
				// DAOUtil.INSTANCE().store(dop);
				// }
				List<DOFormModel> forms = DAOUtil
						.INSTANCE()
						.select(DOFormModel.class,
								"select * from do_ui_formmodel  where relationPropertyUid = ?",
								dop.getObjUid());
				for (Iterator<DOFormModel> itFm = forms.iterator(); itFm
						.hasNext();) {
					DOFormModel fm = itFm.next();
					// if(fm.getLinkService()==null){
					if (fm.getGridModel().getName().toLowerCase()
							.contains("browse")
							|| fm.getGridModel().getName().toLowerCase()
									.contains("_list")
							|| fm.getGridModel().getName().toLowerCase()
									.contains("_result")) {
						fm.setLinkService(aBO.getDSeleByIdService());
						fm.setController(DOController
								.getControllerByName(DOValueResultList.class
										.getCanonicalName()));
					} else {
						fm.setLinkService(aBO.getDSeleAllService());
						fm.setController(DOController
								.getControllerByName(DOResultListPopup.class
										.getCanonicalName()));
					}
