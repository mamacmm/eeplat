...
function callAction(p) {
	...
	if (p.actionName) {
		paras = "callType=as&greenChannel=true&userDefineClass=" + p.actionName
				+ "&" + urlCodeDeal(paras);
	}
	if (p.actionConfigName) {
		paras = "callType=as&contextServiceName=do_auth_owner_browse&greenChannel=true&actionConfigName="
 ...
 * p的定义
 * 
 * p.btn p.serviceUid p.serviceName p.formName p.paras p.pml p.pmlHeight
				...
				success : function(data, textStatus) {
					...
					if (p.pml != null) {
						if (!p.target) {
							...
							$('#' + p.target).loadDefault(p.pml, function(a) {
								$('body').append($('#' + p.target).children());
								html5.close();
							}, true);
						}else{
							$("#" + p.target).empty().wrap("<div/>").parent().loadDefault(p.pml, function(a) {
								$("#" + strId).unwrap("<div/>");
								html5.close();
							}, true);
						}
...
/**
 * p的定义 p.formName p.paras p.target p.pml p.pmlName
 * 
 * @param p
 * @return
 */
function loadPml(p) {
	...
	try {
		if (!p.target) {
			p.target = 'loadpage';
			console.log('loadpml  target::' + p.target);
			console.log('loadpml  paras::' +  urlCodeDeal(paras));
			console.log('loadpml  pml::' +  p.pml);
			$('#' + p.target).loadDefault(p.pml, urlCodeDeal(paras), function(a) {
				if(!p.notpush){
					html5.pushdialog(strId);
				}
			...
			console.log('loadpml  paras::' +  urlCodeDeal(paras));
			console.log('loadpml  pml::' +  p.pml);
			$("#" + p.target).empty().wrap("<div/>").parent().loadDefault(p.pml, urlCodeDeal(paras), function(a) {
				$("#" + strId).unwrap("<div/>");
				if(!p.notpush){
					html5.pushdialog(strId);
				}
...
function pageSplit(dataKey, pmlName, formName) {
	...
	$(" .nextPage")
			.bind(
					...
					function() {
						...
						if (parseInt($("#" + dataKey + " .pageNo").text()) >= parseInt(($("#"
								+ dataKey + " .pageSize").text()))) {
							return;
						}
						var curPageNo = parseInt($("#" + dataKey + " .pageNo")
								.text()) + 1;
	...
	$(" .lastPage").bind(
			...
			function() {
				...
				$.data(document.body, dataKey, $("#" + dataKey + " .pageSize")
						.text());
				var pmlUrl = getPmlUrl(pmlName, $("#" + dataKey + " .pageSize")
						.text(), $("#" + dataKey + " .rowSize").text());
				loadPml({
					'pml' : pmlUrl,
					'pmlName' : pmlName,
					'target' : pmlName,
