package com.exedosoft.plat.ui.jui.grid;
...
public class GridList extends DOViewTemplate {
	...
	public GridList() {
		...
		dealTemplatePath("/grid/GridListPagination.ftl");
	}
	...
	public static List<BOInstance> getListData(DOGridModel gridModel,
		...
		if (DOGlobals.getInstance().getSessoinContext().getFormInstance() != null
				&& DOGlobals.getInstance().getSessoinContext()
						.getFormInstance().getValue("pagenum") != null) {
			try {
				pageNo = Integer.parseInt(DOGlobals.getInstance()
						.getSessoinContext().getFormInstance().getValue(
								"pagenum"));
		...
		if (DOGlobals.getInstance().getSessoinContext()
				.getFormInstance().getValue("numPerPage") != null) {
			pageNum = Integer.parseInt(DOGlobals.getInstance().getSessoinContext().getFormInstance().getValue("numPerPage"));
		}
		if (pageNum <= 0) {
