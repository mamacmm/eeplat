  		   			  ...
  		   			  if(p.target=='_opener_window'){
  						  ...
  						  if($("#" + p.target).size() > 0  ){
								...
								if (pageNo != null && pageSize != null) {
  	  						  ...
  	  						  $("#" + p.target).empty().load(p.pml,getShorterParas(urlCodeDeal(dataParas)));
  						  }else{
  							  $("#" + p.target).empty().load(p.pml,getShorterParas(urlCodeDeal(conParas)));
  						  }
  					  }	
 		   		   }else if(pmlName!=""){

 		   		     $("#" + pmlName).empty().load(p.pml,getShorterParas(urlCodeDeal(conParas)));  
 		   		   }
 		   		   else{
	...
	try{
	   ...
	   if(p.target=='_opener_window'){
			...
			if(p.target=='_opener'){
				...
				if(p.target==pmlName){
					$("#" + pmlName).empty().wrap("<div/>").parent().load(p.pml,getShorterParas(urlCodeDeal(paras)),function(){
						$("#" + pmlName).unwrap("<div/>");
						if(showLoadMsg){
							closeWin();
						}
...
function uploadify(uploadifyID,uploadifyQueueID,fileDesc,fileExt,autoUpload,sessionid,uploadActionFile){
	...
	$("#" + uploadifyID).uploadify({
		...
		'uploader'       : globalURL + 'web/default/' + uploadActionFile + ';jsessionid=' + sessionid , /// /eestorage/    ec2 upload
		'checkExisting'  : globalURL + 'web/default/upload_action_uploadify_exists.jsp;jsessionid=' + sessionid ,
		'auto'           : autoUpload,
		'multi'          : false,
		'simUploadLimit' : 2,
...
function uploadifyMulti(uploadifyID,uploadifyQueueID,fileDesc,fileExt,autoUpload,sessionid,uploadActionFile){
	...
	$("#" + uploadifyID).uploadify({
		'swf'            : globalURL + 'web/default/js/jquery-plugin/fileuploader/uploadify.swf',
		'formData'       : {'jsessionid':sessionid},
		'uploader'       : globalURL + 'web/default/' + uploadActionFile + ';jsessionid=' + sessionid ,
		'checkExisting'  : globalURL + 'web/default/upload_action_uploadify_exists.jsp;jsessionid=' + sessionid ,
