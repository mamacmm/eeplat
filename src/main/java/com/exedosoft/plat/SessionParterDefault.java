...
public class SessionParterDefault implements SessionParter {
	...
	public List getParterAuths(String accountUid) {
		...
		if(user!=null){
			if ("2".equals(user.getValue("asrole"))
					|| "1".equals(user.getValue("asrole"))) {
				OrgParterValue pvRole = new OrgParterValue(roleParter,
						"40288031288a2b8501288a3d009d000d", "admin");
				allAuths.add(pvRole);
			}
		}
				...
				if (findRoleService != null) {
					...
					if (listRoles != null) {
						...
						for (Iterator roles = listRoles.iterator(); roles
							...
							if (listRoles.size() == 1) {
								SessionContext us = SessionContext
										.getInstance();
								us.getUser().putValue("stationuid",
										boRole.getUid());
								us.getUser().putValue("roleuid",
										boRole.getUid());
							}
							allAuths.add(pv);
						}
	...
	public static void main(String[] args) {
		...
		// System.out.println("listRoles::" + listRoles);
		SessionParterDefault spd = new SessionParterDefault();
		List list = spd
				.getMenuAuthConfigByAccount("40288031278ed91501278ed915b30000");

		System.out.println("AuthMenu:::" + list);

	}


	public List getApplicationsByRole(String roleUid) {

		List pureMenuUids = new ArrayList();
		DOService rfService = DOService.getService("s_project_byroleid");
	...
	public List getApplicationsByAccount(String roleUid) {
		// TODO Auto-generated method stub
		return null;
	}
}
