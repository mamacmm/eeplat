...
<div  style="overflow-x:hidden;overflow-y:scroll;width:97%;height:120px;border:1px solid #c0c0c0;position: relative;left:5px;top:-2px;">
	<table id='g${model.objUid}' class='mytablesorter'>
		<#--定义宏 判断输出什么类型的align-->
	   ...
	   <#list data as ins>
			...
			<#if model.checkBox><#--定义CheckBox-->
				<td width="25px;">&nbsp;&nbsp;<input type ='checkbox' value='${ins.uid}' class='list_check'   name='checkinstance'/> <input type ='hidden' value='${ins.uid}' name='checkinstance_hidden'/> </td>
			<#elseif model.radio>
			...
			<#list model.normalGridFormLinks as item>
		        <td <#if item.width?exists>width='${item.width}'</#if> <#if item.noWrapValue>nowrap='nowrap'</#if> <#compress> <@JudgeAlign item/></#compress> >   <#if '${dataBind(ins,item)}' ==''>${item.htmlValue}</#if> </td> 
			</#list>
			</tr>
	     </#list>
