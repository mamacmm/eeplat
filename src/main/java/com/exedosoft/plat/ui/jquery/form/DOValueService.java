...
public class DOValueService extends DOBaseForm  {
	...
	private static StringBuffer stardardOnlyPane(DOFormModel fm, String label,
		...
		buffer.append("\" > <img border=0  src='").append(DOGlobals.PRE_FULL_FOLDER)
				.append("images/search.gif' />").append("</a> </nobr>\n");
		return buffer;
	}
	...
	public static void appendJSPaneLink(DOFormModel fm, StringBuffer buffer) {
		...
		if (fm.getLinkPaneModel().getLinkType()!=null && fm.getLinkPaneModel().getLinkType().intValue() == DOPaneModel.LINKTYPE_RESOURCE) {
			buffer
