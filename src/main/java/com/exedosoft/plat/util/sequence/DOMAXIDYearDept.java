 ...
 * @version  1.0
 */
public class DOMAXIDYearDept implements SequenceBuilder{

	private static DOMAXIDYearDept builder;
	 ...
	 * @return
	 */
	public static DOMAXIDYearDept getInstance() {

		if (builder == null) {
			synchronized (lockObj) {
				if (builder == null) {
					builder = new DOMAXIDYearDept();
				}
			}
		}
		return builder;
	...
	private int getIDFromDb(String codeItemID, String aDeptCode) {
		...
		String selectSql = "SELECT max_sequence FROM do_code_maxsequence WHERE code_ItemUid=? and propertyValue=? and yearSeq=? ";
